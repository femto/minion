...
class BaseLLM(ABC):
    ...
    async def aask(
        ...
        **kwargs,
        ...
        rsp = await self.acompletion_text(message, stream=stream, timeout=self.get_timeout(timeout), **kwargs)
        return rsp
    ...
    @abstractmethod
    async def _achat_completion_stream(self, messages: list[dict], timeout: int = USE_CONFIG_TIMEOUT, **kwargs) -> str:
    ...
    async def acompletion_text(
        self, messages: list[dict], stream: bool = False, timeout: int = USE_CONFIG_TIMEOUT, **kwargs
    ) -> str:
        """Asynchronous version of completion. Return str. Support stream-print"""
        if stream:
            return await self._achat_completion_stream(messages, timeout=self.get_timeout(timeout), **kwargs)
        resp = await self._achat_completion(messages, timeout=self.get_timeout(timeout), **kwargs)
