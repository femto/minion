...
Attributes:
    ...
    MINION_ROOT (Path): Root path for MetaGPT files.
...
#from minion.config2 import merge_dict
from minion.configs.llm_config import LLMConfig
from minion.const import CONFIG_ROOT, MINION_ROOT
#from minion.utils.yaml_model import YamlModel
...
class ModelsConfig(YamlModel):
    ...
    Methods:
        ...
        from_home(cls, path): Loads configuration from ~/.minion/config2.yaml.
    ...
    def from_home(cls, path):
        """
        Loads configuration from ~/.minion/config2.yaml.
    ...
    def default(cls):
        ...
        default_config_paths: List[Path] = [
            MINION_ROOT / "config/config.yaml",
